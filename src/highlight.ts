...
function getLineTagPatternParts(tagDefs: TagDef[]): string[] {
	return sortTagDefsByLengthDesc(tagDefs).map((t) =>
		tagPatternForLine(t.tag, t.escapedTag),
	);
}
...
function matchBlockContentLines(
	...
	for (const { line, startOffset } of lines) {
		...
		list.push(
			new vscode.Range(
				editor.document.positionAt(contentStartOffset),
				editor.document.positionAt(contentEndOffset),
			),
		);
